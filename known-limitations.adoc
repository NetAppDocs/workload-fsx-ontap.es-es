...
---
= Limitaciones conocidas de Amazon FSx para NetApp ONTAP en NetApp Workload Factory
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Las limitaciones conocidas identifican plataformas, dispositivos o funciones que no son compatibles con esta versión del producto o que no interoperan correctamente con él. Las siguientes limitaciones son específicas de Amazon FSx for NetApp ONTAP en NetApp Workload Factory. Revisa estas limitaciones cuidadosamente.
...
== Limitaciones para la replicación de la migración de storage VM

* Workload Factory no admite la replicación de migración para máquinas virtuales de almacenamiento entre FSx de primera y segunda generación para sistemas de archivos ONTAP.
* La transición de replicación no es compatible para volúmenes con puntos de acceso S3 o configuraciones de ONTAP S3, incluyendo object-store-server, buckets, users, groups o policies en el vserver.
