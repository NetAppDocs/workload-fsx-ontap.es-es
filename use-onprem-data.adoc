...
summary: Descubre y replica los datos de ONTAP locales en NetApp Workload Factory para que puedan usarse como copia de seguridad en la nube. 
---
...
[role="lead"]
Descubre y replica los datos de ONTAP locales en NetApp Workload Factory para que puedan usarse como copia de seguridad en la nube.
...
* Un enlace activo para asociarlo al clúster local detectado. Si no tienes un enlace, tendrás que link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["cree uno"^].
...
.. *Credenciales de ONTAP*: Proporciona lo siguiente:
+
*** GovCloud accounts: Introduce ARN con las credenciales ONTAP.
*** Todas las demás cuentas: Introduce las credenciales de ONTAP para el clúster ONTAP local. Asegúrate de que el usuario tiene los permisos necesarios.
...
* Tanto el clúster ONTAP local como el sistema de archivos FSx for ONTAP que utilices para la relación de replicación deben tener un enlace asociado. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["Aprenda a asociar un enlace existente o a crear y asociar un nuevo enlace"^]. Una vez que el enlace esté asociado, vuelve a esta operación.
